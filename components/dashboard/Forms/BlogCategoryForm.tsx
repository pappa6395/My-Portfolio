...
import { BlogCategoryProps, ProjectCategoryProps } from "@/utils/type";
...
const BlogCategoryForm = () => {
    ...
    reset,
    formState: { errors },
    } = useForm<BlogCategoryProps>();

    const router = useRouter();
    const [isLoading, setIsLoading] = useState(false);
    const [success, setSuccess] = useState(false);

    const saveContact = async (data: BlogCategoryProps) => {
        setIsLoading(true);
        data.slug = generateSlug(data.title);

        try {
            
            const newCategory = await createBlogCategory(data)
            reset()
            setSuccess(true)
            setTimeout(() => {
